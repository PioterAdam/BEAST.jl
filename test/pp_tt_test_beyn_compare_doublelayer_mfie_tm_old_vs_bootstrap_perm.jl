...
# ============================================================
# Helpers for recovering permutation from permuted basis
# ============================================================

function dof_signature(fs)
    return Tuple((getfield(sh, 1), getfield(sh, 2), getfield(sh, 3)) for sh in fs)
end

function basis_permutation(Xorig, Xperm)
    fns_orig = getfield(Xorig, 2)
    fns_perm = getfield(Xperm, 2)

    dict = Dict{Any,Int}()
    for (i, fs) in enumerate(fns_orig)
        dict[dof_signature(fs)] = i
    end

    p = Vector{Int}(undef, length(fns_perm))
    for (j, fs) in enumerate(fns_perm)
        p[j] = dict[dof_signature(fs)]
    end
...
function make_ctx_circle(; radius::Float64=1.0, Nel::Int=512, minvalues::Int=100)
    ...
    return CircleDLKCtx(X, tree, Iop)
end
...
function make_ctx_circle_perm_bootstrap(; radius::Float64=1.0,
    ...
    ptest  = basis_permutation(ctx.X, Xtest_p)
    ptrial = basis_permutation(ctx.X, Xtrial_p)

    Iop_perm = ctx.Iop[ptest, ptrial]

    return CircleDLKPermCtx(ctx.X, Xtest_p, Xtrial_p, ctx.tree,
                            ptest, ptrial, ctx.Iop, Iop_perm)
end
...
build_dense_Iop(ctx::CircleDLKCtx) = ctx.Iop
build_dense_Iop(ctx::CircleDLKPermCtx) = ctx.Iop_perm
