...
#using Krylov
#=
function test_circle_cavity_Dirichlet_Beyn_FMM()

    println("=== Beyn eigenvalue test: circular cavity (Dirichlet, interior) — FMM/ACA ===")
    ...
    # Dimension (cheap, no dense assembly)
    D = size(ctx.I, 1)
    @printf("D = %d\n", D)

    # -----------------------
    # Target eigenvalue (analytic for disk Dirichlet: j_{0,1}/a)
    # -----------------------
    j01    = besselj_zero(0, 1)
    k_true = j01 / a
    @printf("Analytic k_{0,1} ≈ %.10f\n", k_true)

    println("\n--- Diagnostic scan along real k ---")
    kgrid = range(2.2, 2.6; length=25)
    b = randn(ComplexF64, D)

    for k in kgrid
        kk = ComplexF64(k)
        Tk = (-0.5)*ctx.I + getK!(ctx, kk)
        x, st = Krylov.gmres(Tk, b; rtol=1e-10, itmax=2000, verbose=0)
        @printf("k=%.4f  solved=%s  it=%d  ||x||=%.3e\n",
                k, st.solved, st.niter, norm(x))
    end
    println("--- end scan ---\n")
    ...
    # -----------------------
    # Operator hooks for Beyn
    # -----------------------
    UserFun = (k, Vhat, MIV) -> Beyn_circle_D(k, Vhat, MIV, ctx; rtol=1e-8, itmax=400, verbose=0)
    ApplyT  = (k, v) -> begin
        kk = ComplexF64(k)
        ((-0.5) * ctx.I + getK!(ctx, kk)) * v
    end

    # -----------------------
    # Optional quick consistency sanity-check at k_true (self-consistent operator)
    # -----------------------
    begin
        Kk = getK!(ctx, z0)
        Tk = (-0.5) * ctx.I + Kk
        μt = randn(ComplexF64, D)
        gt = Tk * μt
        μg, st = Krylov.gmres(Tk, gt; rtol=1e-10, itmax=400, verbose=0)
        @printf("Self-check GMRES rel.err ≈ %.3e (solved=%s)\n",
                norm(μg - μt) / norm(μt), st.solved)
    end
...
λ_fmm, V_fmm, ok_fmm = test_circle_cavity_Dirichlet_Beyn_FMM()
=#
