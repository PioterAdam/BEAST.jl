println("RUNNING FILE: rr_test_beyn_compare_singlelayer_old_vs_bootstrap_perm.jl")
...
function run_beyn_dense_singlelayer(;
        radius::Float64 = 1.0,
        Nel::Int = 512,
    ...
    println("=== DENSE Beyn / single-layer / EFIE ===")
    ...
    UserFun = (k, Vhat, MIV) -> begin
        S = assemble(Helmholtz2D.singlelayer(; wavenumber=k), X, X; quadstrat=quadstrat)
        MIV .= Matrix(S) \ Vhat
        MIV
    end
...
function run_beyn_aca_original_singlelayer(;
        radius::Float64 = 1.0,
        Nel::Int = 512,
        minvalues::Int = 100,
        tol::Float64 = 1e-4,
    ...
    println("=== ACA Beyn / original basis / single-layer / EFIE ===")
...
function run_beyn_aca_bootstrap_perm_singlelayer(;
        radius::Float64 = 1.0,
        Nel::Int = 512,
        minvalues::Int = 100,
        tol::Float64 = 1e-4,
    ...
    UserFun = (k, Vhat, MIV) -> Beyn_circle_D_ACA(k, Vhat, MIV, ctxp;
        tol=tol, η=η, scheduler=scheduler,
        nearquadstrat=nearquadstrat, farquadstrat=farquadstrat,
        gmres_rtol=gmres_rtol, gmres_itmax=gmres_itmax, verbose=false)

    ApplyT = (k, v) -> ApplyT_circle_D_ACA(k, v, ctxp;
        tol=tol, η=η, scheduler=scheduler,
        nearquadstrat=nearquadstrat, farquadstrat=farquadstrat)
...
function run_compare_singlelayer_beyn()
    radius = 1.0
    Nel = 512

    # keep same contour for all three runs
    ...
    println("\n================ DENSE =================\n")
    λd, Vd, okd = run_beyn_dense_singlelayer(
        radius = radius,
        Nel = Nel,
        z0 = z0,
        Rx = Rx,
    ...
    println("\n================ ACA ORIGINAL BASIS =================\n")
    λa0, Va0, oka0 = run_beyn_aca_original_singlelayer(
        radius = radius,
        Nel = Nel,
        minvalues = 100,
        tol = tol,
    ...
    λap, Vap, okap = run_beyn_aca_bootstrap_perm_singlelayer(
        radius = radius,
        Nel = Nel,
        minvalues = 100,
        tol = tol,
...
λd, λa0, λap = run_compare_singlelayer_beyn()